...
function onLoad(assignmentId, ownerName, lastChanged, piazzaReleasePostUrl) {
  // Insert generic text
  const header = $('#assignmentHeader');

  header.append($('<div>')
    .append($('<div>', {class: 'assignmentTitle'}).append(document.title))
    .append($('<div>').append('Stanford CS221 Autumn 2020-2021'.bold())));
  header.append($('<p>').append('Owner CA: ' + ownerName));
  header.append($('<p>').append('Last Changed: ' + lastChanged));
  ...
  header.append($('<p>').append("All written answers must be <b>typeset (preferably in LaTeX)</b>. We strongly recommend using Overleaf: a link to a starter guide and a LaTeX written answer template is provided on the main course page."));

  header.append($('<p>').append("<b> WARNING: </b> As noted on the course webpage from the start of the quarter, we require that all solutions be typeset. To enforce this policy, starting with Homework 4 non-typeset submissions will not be accepted. <b> If your submission is not typeset, we will request that you typeset it and re-submit, possibly using late days.</b>"));
  ...
  if (piazzaReleasePostUrl !== undefined) {
    header.append($('<p>').append(
    '<b>Finally, before starting (and later, before submitting) we recommend you check out the <a href="' + piazzaReleasePostUrl + '" target="_blank">release Piazza post</a> for any clarifications or updates.<b>'));
  }
